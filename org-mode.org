...
~gcr-org-latex-packages-alist-post-hyperref~ for what must come after. For that to work you have to remove =hyperref= from the default package list which is only recommended if you really know what you are doing.
